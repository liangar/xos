...
//*/
	...
	// printf("will accept (%d)\n", int(SockFD)); 
	int *SockALen = iNameLen;
...
//  Set send timeout
///////////////////////////////////////////////////////////////////////////////
	int OptLenght = sizeof(struct timeval);
...
///////////////////////////////////////////////////////////////////////////////
	...
	while (ulCurrOffset < ulSndEndOffset) {
		...
		unsigned long ulSendSize = (unsigned long) sendfile((int) SockFD, iFileID,
								    (off_t) &ulStartOffset, ulToSend, (struct iovec *) NULL, 0);
		    ...
		    NET_ADDRESS const &NetAddr, int iPortNo)
{
	...
	SAIN_Addr(AddrInfo.Addr) = (SYS_UINT32)NetAddr;
...
//	    (pHostEnt->h_addr_list[0] == NULL)) {
	...
	//if ( (gethostbyname_r(pszName, &HostEnt, &hdData) != 0) 
	//	|| (pHostEnt == NULL) ||  (pHostEnt->h_addr_list[0] == NULL)) {
...
//	    (pHostEnt == NULL) || (pHostEnt->h_name == NULL)) {
	...
	pHostEnt = gethostbyaddr((const char *) &SAIN_Addr(AddrInfo.Addr), sizeof(SAIN_Addr(AddrInfo.Addr)), AF_INET);
...
int SysGetPeerInfo(SYS_SOCKET SockFD, SYS_INET_ADDR & AddrInfo)
{
	...
	int InfoSize = sizeof(AddrInfo.Addr);
...
int SysWaitSemaphore(SYS_SEMAPHORE hSemaphore, int msec)
{
...
int SysWaitEvent(SYS_EVENT hEvent, int msec)
{
	EventData *pED = (EventData *) hEvent;
	...
	if (iTimeout == SYS_INFINITE_TIMEOUT) {
		...
		tsTimeout.tv_sec = tvNow.tv_sec + msec/1000;
		tsTimeout.tv_nsec = (tvNow.tv_usec + (msec%1000)*1000) * 1000;
...
*/
	...
	*pRamTotal = (SYS_INT64) psd.psd_rm * page_size;
	*pRamFree = (SYS_INT64) 0;
	*pVirtTotal = (SYS_INT64) psd.psd_vm * page_size;
	*pVirtFree = (SYS_INT64) psd.psd_free;
...
int SysDoCmd(const char * pszCmmand, const char * workpath, char * outbuf, int len)
{
	...
	if ((pfp = popen(pszCmmand, "r")) == NULL)
	{
		if (outbuf && len > 0)
		{
			strncpy(outbuf, "Cannot execute the command", len);  outbuf[len-1] = '\0';
		}
		return -1;
	}
	int l = 0;
	if (outbuf && len > 0)
	{
		l = fread(outbuf, 1, len-1, pfp);  outbuf[len-1] = '\0';    
	}
	pclose(pfp);
	return l;
}
