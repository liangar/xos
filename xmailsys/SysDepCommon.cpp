...
int SysGetHostByName(const char *pszName, int iFamily, SYS_INET_ADDR &AddrInfo)
{
    ...
    struct hostent  *hp;
	sockaddr_in *p = (sockaddr_in *)AddrInfo.Addr;

	AddrInfo.iSize = sizeof(sockaddr_in);
    memset(p, 0, sizeof(sockaddr_in));
    p->sin_family = AF_INET;

    if( (hp = gethostbyname(pszName)) == NULL)
        return WSAGetLastError();
    memcpy(&p->sin_addr, hp->h_addr, hp->h_length);
...
{
	char * p = SysInetNToA(AddrInfo, pszFQDN, iSize);
	if (!p[0]){
		ErrSetErrorCode(ERR_INVALID_INET_ADDR);
		return ERR_INVALID_INET_ADDR;
	}
...
char *SysInetNToA(SYS_INET_ADDR const &AddrInfo, char *pszIP, int iSize)
{
	...
	struct in_addr * p = (struct in_addr *)AddrInfo.Addr;
