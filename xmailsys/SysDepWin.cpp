...
int SysConnect(SYS_SOCKET SockFD, const SYS_INET_ADDR *pSockName, int iTimeout)
{
	...
	if (iConnectError == WSAEISCONN){
		iConnectError = ERR_CONNECT;
		printf("already connected\n");
	}else if (iConnectResult != 0 && iConnectError == WSAEWOULDBLOCK) {
...
int SysDoCmd(const char * pszCmmand, const char * workpath, char * outbuf, int len)
{
	...
	// create pipe for ipc
	HANDLE hread, hwrite;
	hread = hwrite = SYS_INVALID_HANDLE;
	...
	if (outbuf && len > 1)
		CreatePipe(&hread, &hwrite, &sa, 0); // Create anonymous pipe and get both ends
	...
	try{
		...
		s.cb = sizeof(s);
		s.dwXSize = STARTF_USESIZE;
		s.dwYSize = STARTF_USESIZE;
		s.dwXCountChars = STARTF_USECOUNTCHARS;
		s.dwYCountChars = STARTF_USECOUNTCHARS;
		s.dwFillAttribute=STARTF_USEFILLATTRIBUTE;
		s.dwFlags = STARTF_USESHOWWINDOW;
		s.wShowWindow = SW_HIDE;

		s.dwFlags = s.dwFlags | STARTF_USESTDHANDLES;
		if (outbuf && len > 1)
			s.hStdOutput = hwrite;
		else
		...
		if (!CreateProcess(0,
			...
			&info))
		{
			throw "Can not execute the command.";
		}

		// wait for return
		if (outbuf && len > 1){
			DWORD dwResult;
			if ((dwResult = WaitForSingleObject(info.hThread, 30 * 1000)) != WAIT_OBJECT_0){
				TerminateProcess(info.hProcess, 0);
				throw "The process execute timeout(>30)";
			}
		}else
			r = 1;
	}catch (const char * msg){
		if (outbuf && len > 1){
			strncpy(outbuf, msg, len);  outbuf[len-1] = '\0';
		}
		r = -1;
	}catch (...){
		r = -2;
	...
	if (hwrite != SYS_INVALID_HANDLE)
		CloseHandle(hwrite);

	if (hread != SYS_INVALID_HANDLE){
		// get result
		DWORD dwBytesRead;
		if (!ReadFile(hread, outbuf, len-1, &dwBytesRead, NULL)){
			r = -3;
		}else{
