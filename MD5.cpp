...
void  *md5_read_ctx(const struct md5_ctx * ctx, void *resbuf)
{
    ...
    ((md5_uint32 *) resbuf)[0] = MD5_SWAP(ctx->A);
    ((md5_uint32 *) resbuf)[1] = MD5_SWAP(ctx->B);
    ((md5_uint32 *) resbuf)[2] = MD5_SWAP(ctx->C);
    ((md5_uint32 *) resbuf)[3] = MD5_SWAP(ctx->D);
...
void  *md5_finish_ctx(struct md5_ctx * ctx, void *resbuf)
{
 ...
 /* Put the 64-bit file length in *bits* at the end of the buffer.  */
    *(md5_uint32 *) & ctx->buffer[bytes + pad] = MD5_SWAP(ctx->total[0] << 3);
    *(md5_uint32 *) & ctx->buffer[bytes + pad + 4] = MD5_SWAP((ctx->total[1] << 3) |
   (ctx->total[0] >> 29));
...
#define OP(a, b, c, d, s, T)		\
	do								\
	{								\
		a += FF (b, c, d) + (*cwp++ = MD5_SWAP (*words)) + T;		\
		++words;					\
		CYCLIC (a, s);				\
		a += b;						\
	}while (0)						\
...
void   do_md5_string(const char *pass, int passlen, char *hash)
{
    struct md5_ctx  ctx;
